    ...
    def _validate_against_core_values(self, recommendations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for rec in recommendations:
            ...
            if rule['insurance_type'] == '年金':
                has_basic_coverage = any(r['insurance_type'] in ['寿险', '健康', '意外'] for r in recommendations)
...
**建议关注时机**：当家庭成员结构变化（如新增子女）或收入显著提升时，可重新评估寿险需求。""",
            ...
            '健康': """**暂不推荐理由：**
1. **已有保障评估**：您已拥有较为完善的医保或其他健康保障，重复配置的边际效益较低
2. **健康状况限制**：根据您的健康告知，部分健康险产品可能无法承保或需加费
3. **预算优先级**：建议优先配置其他更急需的保障类型

**建议**：定期审视健康状况和保障缺口，适时补充健康险保障。""",
